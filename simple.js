...
_*JIKA BUTTON TIDAK TERLIHAT KETIK .allmenu*_
 ...
 but = [
          ...
          { buttonId: `${prefix}allmenu`, buttonText: { displayText: '〔-ALLMENUI-〕' }, type: 1 },
          { buttonId: `${prefix}userinfo`, buttonText: { displayText: '〔-YOUR INFO-〕' }, type: 1 },
        ]
...
reply(mess.wait)
await fetchJson(`https://api.lolhuman.xyz/api/ytsearch?apikey=14a6072c1a48c7595c5fbce1&query=${args.join(" ")}`)
.then(async(result) => {
...
teks =`
*Hai Kak ${pushname}*
_Jika Button Tidak Terlihat Pilih Salah Satu Menunya_
•々${prefix}original
•々${prefix}catalog`
but = [
{ buttonId: `${prefix}catalog`, buttonText: { displayText: 'ALL1'}, type: 1 },
{ buttonId: `${prefix}original`, buttonText: { displayText: 'ALL2'}, type: 1 },
]
sendButton(from, teks, faketeks, but) 
break
...
simple.sendMessage(from, tod, but, {quoted: { key : { participant : `0@s.whatsapp.net`, "remoteJid":  '6283136505591-1614953337@g.us', "fromMe": false, "id": "B391837A58338BA8186C47E51FFDFD4A" }, "message": { "documentMessage": { "jpegThumbnail": fs.readFileSync('./poto/thub_1.jpg'), "mimetype": "application/octet-stream","title": `${setting.owner}`, "fileLength": "36", "pageCount": 0, "fileName": `${botname}`}}, "messageTimestamp": "1614069378", "status": "PENDING"},contextInfo:{"forwardingScore":999,"isForwarded":true},sendEphemeral: true})
break
...
case 'brainly':
  if (args.length < 1) return reply('Pertanyaan apa')
					...
					teks += `\n*「 _Brainly_ 」*\n\n*乂. Pertanyaan:* ${Y.pertanyaan}\n\n*乂. Jawaban:* ${Y.jawaban[0].text}\n====================[ *BRAINLY* ]======================\n`
					}
					simple.sendMessage(from, teks, text,{quoted:ftrol,detectLinks: false})                        
		            })              
					break
...
case 'cecan':
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fgif})
getBuffer(`https://api.lolhuman.xyz/api/random/cecan?apikey=${humankey}`).then((gambar) => {
simple.sendMessage(from, gambar, image, { quoted: ftrol })
})
break
...
case 'cogan':
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fgif})
getBuffer(`https://api.lolhuman.xyz/api/random/cogan?apikey=${humankey}`).then((gambar) => {
simple.sendMessage(from, gambar, image, { quoted: ftrol })
})
break
